...
{
    public class Daily1Key<T> : IDisposable
    {
        ...
        public void AllocateMemory(IEnumerable<int> completeKeyIDs, DateTime startDate, DateTime endDate)
        {
            StartDate = startDate;
            EndDate   = endDate;
            KeyIDs    = completeKeyIDs.OrderBy(x => x).ToArray();
            ...
            for (int y = 0; y < Data.Length; y++)
            {
                ...
                for (int m = 0; m < Data[y].Length; m++)
                {
                    ...
                    for (int d = 0; d < Data[y][m].Length; d++)
                        Data[y][m][d] = new T[KeyIDs.Length];
                }
            }
            IsAllocated = true;
        }
        ...
        public IEnumerable<T> this[DateTime date]
        {
            get
            {
                ...
                {
                    if (Data[y][m][d][i] != null)
                        ret.Add(Data[y][m][d][i]);
                }
                return ret;
            }
        }
        ...
        {
            get { return Get(year, month, day, keyID); }
            set { Set(year, month, day, keyID, value); }
        }


        public T Get(DateTime date, int key, bool errorIfNull = false)
            => Get(date.Year, date.Month, date.Day, key, errorIfNull);


        public T Get(int year, int month, int day, int key, bool errorIfNull = true)
        {
            var y = year;
            var m = month;
            var d = day;
            ...
            if (ret == null && errorIfNull) throw new IndexOutOfRangeException
